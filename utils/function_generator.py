...
def tepmlate_mem_function(out,arg_num,funattr):
	...
	out.write('    T1* ptr = (T1*)luaL_testudata(state, 1, types::metatable_name<T1>().c_str());\n') 
	out.write('    if(!ptr) ptr = ((standard::shared_ptr<T1>*)luaL_testudata(state, 1, types::metatable_name<standard::shared_ptr<T1> >().c_str()))->get();//fallback to shared_ptr<T>\n') 
